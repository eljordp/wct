      ...
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            <Leaf className="w-6 h-6 text-[#39FF14]" />
            <h1 className="text-3xl sm:text-4xl font-bold">Our Menu</h1>
          </div>
          <p className="text-gray-500">Explore our curated menu filtered by terpene profiles. Find the perfect match for your vibe.</p>
        </motion.div>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-medium text-gray-400">Terpene Profile</span>
          </div>
        ...
        {filtered.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            {filtered.map((product, i) => (<ProductCard key={product.id} product={product} index={i} onAddToCart={addToCart} />))}
          </div>
        ) : (
          <div className="text-center py-20">
            <Leaf className="w-12 h-12 text-gray-700 mx-auto mb-4" />
            <p className="text-gray-500 mb-2">No products match your filters</p>
            <button onClick={() => { setSearch(''); setCategory('all'); setProfile('all') }} className="text-sm text-[#39FF14] hover:underline">Clear filters</button>
          </div>
        )}
